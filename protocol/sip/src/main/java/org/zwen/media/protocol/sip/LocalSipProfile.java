/*
 * This file is part of TinySip. http://code.google.com/p/de-tiny-sip/ Created 2011 by Sebastian
 * R�sch <flowfire@sebastianroesch.de> This software is licensed under the Apache License 2.0.
 */
...
/**
 * Represents the local user's profile, containing his user name, the sip domain and so on. Also
 * contains information about user authorization with the sip provider.
 * @author Sebastian
 */
public class LocalSipProfile {
    ...
    public SipURI createSipUrI(AddressFactory addressFactory) {
        String uri;
        uri = String.format("sip:%s@%s:%s", userName, sipDomain, sipPort);
        try {
            return (SipURI)addressFactory.createURI(uri);
        } catch (Exception e) {
            throw new IllegalArgumentException(uri + ", " + e.getMessage(), e);
        }
    }
     ...
     */
    public FromHeader getFromHeader(AddressFactory addressFactory, HeaderFactory headerFactory)
     ...
     * @throws ParseException
     */
    public ToHeader getToHeader(AddressFactory addressFactory, HeaderFactory headerFactory)
            throws PeerUnavailableException, ParseException {
        SipURI fromAddress = addressFactory.createSipURI(this.userName, this.sipDomain);
        Address fromNameAddress = addressFactory.createAddress(fromAddress);
        fromNameAddress.setDisplayName(this.displayName);
     ...
     * @throws ParseException
     */
    public AuthorizationHeader getAuthorizationHeader(HeaderFactory headerFactory)
            throws ParseException {
        SipUri uri = new SipUri();
        uri.setHost(this.sipDomain);

        String responseDigest =
                AuthorizationDigest.getDigest(this.userName,
                                              this.realm,
                                              this.sipPassword,
                                              "REGISTER",
                                              uri.toString(),
     ...
     * @param headerFactory an instance of a HeaderFactory, created by the SipManager
     * @param contact the SipContact to call
     * @return the ProxyAuthorizationHeader for this LocalSipProfile
     * @throws ParseException
     */
    public ProxyAuthorizationHeader getProxyAuthorizationHeader(HeaderFactory headerFactory)
            throws ParseException {
        SipUri uri = new SipUri();
        uri.setHost(this.sipDomain);
     ...
     * @param audioFormats the supported audio formats as a List of SipAudioFormat
     */
     ...
     * @param videoFormats the supported audio formats as a List of SipVideoFormat
     */
     ...
     * @param nonce the nonce to use for authentication
     */
