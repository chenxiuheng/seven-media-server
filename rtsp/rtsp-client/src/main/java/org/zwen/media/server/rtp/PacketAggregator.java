...
public class PacketAggregator implements RtpSessionDataListener {
	private static Logger logger = LoggerFactory.getLogger(PacketAggregator.class);
	...
	public PacketAggregator(int capacity) {
		JitterBuffer b = null;
	...
	public void dataPacketReceived(RtpSession session,
			RtpParticipantInfo participant, DataPacket buffer) {
		assertLocked(buffer);
		if (noMoreFree())
		...
		if (firstSeq == Buffer.SEQUENCE_UNKNOWN
			...
			if (bufferSN >= firstSeq - getFreeCount()) {
				...
				buffer.setSequenceNumber(bufferSN % 65535 + 65535);
				append(buffer);
			} else {
				returnFree(buffer);
				return;
